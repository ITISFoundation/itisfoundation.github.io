  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxapp.store.Store", {
    ...
    members: {
      ...
      __matchPortType: function __matchPortType(typeA, typeB) {
        if (typeA === typeB) {
          return true;
        }

        var mtA = qxapp.data.MimeType.getMimeType(typeA);
        var mtB = qxapp.data.MimeType.getMimeType(typeB);
        return mtA && mtB && new qxapp.data.MimeType(mtA).match(new qxapp.data.MimeType(mtB));
      },
      areNodesCompatible: function areNodesCompatible(topLevelPort1, topLevelPort2) {
        console.log("areNodesCompatible", topLevelPort1, topLevelPort2);
        return topLevelPort1.isInput !== topLevelPort2.isInput;
      },
      arePortsCompatible: function arePortsCompatible(port1, port2) {
        return port1.type && port2.type && this.__matchPortType(port1.type, port2.type);
      },
      getNodeMetaData: function getNodeMetaData(key, version) {
        var metaData = null;
        ...
        if (key && version) {
          ...
          var allServices = this.getFakeServices().concat(this.getBuiltInServices());
      ...
      getBuiltInServices: function getBuiltInServices() {
        var builtInServices = [{
      ...
      getBuiltInServices2: function getBuiltInServices2() {
        var builtInServices = [{
          ...
          inputs: {
            ...
            mapper: {
              ...
              defaultValue: [{
                "Neumann-UUID": ["Plane X+", "Plane X-", "Plane Y+", "Plane Y-", "Plane Z+", "Plane Z-"]
              }]
            }
          },
          outputs: {
          ...
          inputs: {
            ...
            mapper: {
              ...
              defaultValue: [{
                "Field-Sensor-UUID": ["Overall Field"]
              }]
            }
          },
          outputs: {
      ...
      getFakeServices: function getFakeServices() {
        return qxapp.dev.fake.Data.getFakeServices();
      },
      getServices: function getServices(reload) {
        var _this = this;

        if (!this.__reloadingServices && (reload || Object.keys(this.__servicesCached).length === 0)) {
          this.__reloadingServices = true;
          var req = new qxapp.io.request.ApiRequest("/services", "GET");
          req.addListener("success", function (e) {
            var requ = e.getTarget();

            var _requ$getResponse = requ.getResponse(),
                data = _requ$getResponse.data;

            var allServices = data.concat(_this.getBuiltInServices());
            var filteredServices = qxapp.utils.Services.filterOutUnavailableGroups(allServices);
            var services = qxapp.utils.Services.convertArrayToObject(filteredServices);

            _this.__servicesToCache(services, true);
          }, this);
          req.addListener("fail", function (e) {
            ...
            var allServices = _this.getFakeServices().concat(_this.getBuiltInServices());

            var filteredServices = qxapp.utils.Services.filterOutUnavailableGroups(allServices);
            var services = qxapp.utils.Services.convertArrayToObject(filteredServices);

            _this.__servicesToCache(services, false);
          }, this);
          req.send();
          return null;
        }
      ...
      __servicesToCache: function __servicesToCache(services, fromServer) {
        this.__servicesCached = {};
      ...
      __addCategoryToServices: function __addCategoryToServices(services) {
        var cats = {
        ...
        for (var serviceKey in services) {
          if (Object.prototype.hasOwnProperty.call(services, serviceKey)) {
            ...
            if (serviceKey in cats) {
              for (var version in service) {
                var serv = service[version];
              ...
              for (var _version in service) {
                service[_version]["category"] = "Unknown";
              }
            }
          }
        }
      },
      getItemList: function getItemList(nodeKey, portKey) {
        return qxapp.dev.fake.Data.getItemList(nodeKey, portKey);
      },
      getItem: function getItem(nodeInstanceUUID, portKey, itemUuid) {
        return qxapp.dev.fake.Data.getItem(nodeInstanceUUID, portKey, itemUuid);
      },
      stopInteractiveService: function stopInteractiveService(nodeId) {
        var url = "/running_interactive_services";
        var query = "/" + encodeURIComponent(nodeId);
        var request = new qxapp.io.request.ApiRequest(url + query, "DELETE");
        request.send();
      }
    }
  });
  qxapp.store.Store.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Store.js.map?dt=1568886164444